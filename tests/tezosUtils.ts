...
export class TezosEscrowClient {
     ...
     */
    async fundDstEscrow(params: {dstAmount: string; expirationDuration: number; secretHash: string}): Promise<string> {
        try {
            // Mock implementation for testing
     ...
     */
    async claimFunds(params: {orderId: number; secret: string}): Promise<string> {
        try {
            // Mock implementation for testing
...
export function createTezosWalletFromEthKey(ethPrivateKey: string): {
    ...
    const cleanPrivateKey = ethPrivateKey.startsWith('0x') ? ethPrivateKey.slice(2) : ethPrivateKey
...
export function validateEscrowParams(params: {
    ...
    const required = ['orderHash', 'hashlock', 'maker', 'taker', 'token', 'amount', 'safetyDeposit', 'timelocks']
